...
`release: version 1.1 - 2014-11-27`
...
*--store*::
	 ...
	 stores valid invocation sequence in `${XDG_CONFIG_DIR:-$HOME/.config}/.setroot-restore`

*--restore*::

	 looks for `${XDG_CONFIG_DIR:-$HOME/.config}/setroot-restore` and calls invocation sequence (restores your previously set wallpapers and options)
