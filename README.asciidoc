...
`release: version 1.3 - 2014-12-11`
...
*--store*::
	 ...
	 stores valid invocation sequence in file `.setroot-restore` located in directory `${XDG_CONFIG_HOME:-$HOME/.config}/setroot`
...
`setroot` creates the directory `${XDG_CONFIG_HOME:-$HOME/.config}/setroot` and places the file `.setroot-restore` there when `setroot --store` is invoked for the first time. `setroot --restore` uses this file to restore the most recent (valid) invocation of `setroot`.
